...
let
  ...
  cfg = config.services.mystrongswan;
  ...
  strongswanConf = {setup, connections, ca, secrets, pkcsPath}: toFile "strongswan.conf" ''
    charon {
      plugins {
        stroke {
...
in
{
  options.services.mystrongswan = {
    enable = mkEnableOption "strongSwan";

    pkcsPath = mkOption {
      type = types.path;
      default = "${pkgs.opensc}/lib/libopensc.so";
    };
  ...
  config = with cfg; mkIf enable {
    systemd.services.strongswan = {
      ...
      environment = {
        STRONGSWAN_CONF = strongswanConf { inherit setup connections ca secrets pkcsPath; };
      };
      serviceConfig = {
