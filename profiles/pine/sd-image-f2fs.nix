...
in
{
  ...
  system.build.sdImage = pkgs.callPackage ({ stdenv, dosfstools, e2fsprogs,
    mtools, libfaketime, fakeroot, util-linux, f2fs-tools }: stdenv.mkDerivation {
      name = imageName;

      nativeBuildInputs = [ dosfstools e2fsprogs mtools libfaketime fakeroot util-linux f2fs-tools ];
      ...
      buildCommand = ''
        ...
        echo "Calculated size for partition: $bytes bytes"
        echo "numInodes: $numInodes | numDataBlocks: $numDataBlocks"

        echo "Creating f2fs filesystem"
        truncate -s $bytes ./root.img

        mkfs.f2fs -f -l ${imageLabel} \
          -O extra_attr,inode_checksum,sb_checksum,compression \
          ./root.img

        echo "Copying files into rootfs"
        sload.f2fs -f ./rootImage ./root.img
        ...
        dd conv=notrunc,sparse if=./root.img of=$img seek=$START count=$SECTORS
